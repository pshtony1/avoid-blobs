...
function attackRange()
{
    ...
    ctx.strokeStyle = "rgba("+attackLineColor[0]+", "+attackLineColor[1]+", 0, "+attackLineColor[2]+")";
    if (attackOption == 1 || attackOption == 2)
    {
        ...
        ctx.lineTo(attackX * scale + canvas.width / 2, attackY * scale + canvas.height / 2);
...
function attackDelay()
{
    ...
    // red
    if (attackLineColor[1] == 0)
        attackLineColor[1] = 255;

    // yellow
    else if (attackLineColor[1] == 255)
        attackLineColor[1] = 0;
...
function destroyAttackBall()
{
    for (var i = 0; i < attackBallX.length; i++)
    {
        if (!(attackBallX[i] < canvas.width && attackBallX[i] > 0 && attackBallY[i] < canvas.height && attackBallY[i] > 0))
        {
...
function endPage()
{
    if (newBest)
    {
        ...
        if (newBestEffectAlpha > 0)
        {
            ...
            ctx.lineWidth = 6;
            ctx.beginPath();
            ctx.arc(canvas.width / 2, canvas.height / 2 - 100, newBestEffectSize, 0, Math.PI * 2);
            ctx.stroke();
            ctx.closePath();
    ...
    ctx.fillText("Press Space to Main.", canvas.width / 2, canvas.height / 2 + 230);
...
function bestTime()
{
    ...
    if (start == 0)
    {
        ...
        ctx.fillText("Best Time: " + sessionStorage.getItem("bestTime"), canvas.width / 2, canvas.height / 2 - 100);
    }
    ...
    else if (start == 1)
    {
        ...
        if (lastTime <= sessionStorage.getItem("bestTime"))
            ctx.fillText("Best Time: " + sessionStorage.getItem("bestTime"), canvas.width / 2, canvas.height / 2 - 230);

        else
            ctx.fillText("Best Time: " + lastTime, canvas.width / 2, canvas.height / 2 - 230);
    }
...
function startText()
{
    ...
    if (customized == 0)
    {
        ctx.fillText("Customize your Frame Size.", canvas.width / 2, canvas.height / 2 - 190);
        ctx.beginPath();
        ctx.moveTo(canvas.width / 4 + 35, canvas.height / 2 - 13);
        ctx.lineTo(canvas.width / 4 + 35, canvas.height / 2 + 13);
        ctx.lineTo(canvas.width / 4 + 35 - (20 * Math.sqrt(3) / 2), canvas.height / 2);
        ctx.fill();
        ctx.moveTo(canvas.width * 3 / 4 - 35, canvas.height / 2 - 13);
        ctx.lineTo(canvas.width * 3 / 4 - 35, canvas.height / 2 + 13);
        ctx.lineTo(canvas.width * 3 / 4 - 35 + (20 * Math.sqrt(3) / 2), canvas.height / 2);
        ctx.fill();
    }
    ...
    else
    {
        if (tooLarge == 0)
            ...
            ctx.fillText("Too Large!", canvas.width / 2, canvas.height / 2 - 190);

        ctx.fillText("Press R to Initiallize.", canvas.width / 2, canvas.height / 2 + 100);
    }
    ...
    ctx.fillText("Press Space to Start.", canvas.width / 2, canvas.height / 2 + 230);
}
...
function drawTime()
{
    ...
    // 소수점 1자리까지만 있다면.
    else if (time * 10 == Math.floor(time * 10))
        time += "0";
...
function drawEnemy()
{
    for (var i = 0; i < randX.length; i++)
    {
        ...
        ctx.fillStyle = "rgb("+enemyColor[i][0]+", "+enemyColor[i][1]+", "+enemyColor[i][2]+")";
...
function setEnemy()
{
    ...
    enemyColor.push([Math.random() * 255, Math.random() * 255, Math.random() * 255]);
...
function checkDead()
{
    ...
    for (var i = 0; i < randX.length; i++)
    {
        ...
        if (dist + 10 < blockSize * Math.sqrt(2) / 2 + enemySize[i] * sizeFactor[i])
        {
            start = 2;
        }
    }
...
function drawEffect()
{
    ...
    {
        ctx.strokeStyle = "rgba("+effectColor[i][0]+", "+effectColor[i][1]+", "+effectColor[i][2]+", "+effectColor[i][3]+")";
        ctx.lineWidth = 6;
        ctx.beginPath();
